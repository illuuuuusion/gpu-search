...
const KNOWN_BOARD_BRANDS = new Map([
    ['aorus', 'AORUS'],
    ['asrock', 'ASRock'],
    ['asus', 'ASUS'],
    ['colorful', 'Colorful'],
...
function detectBoardModel(input) {
    ...
    for (const text of [input.subtitle, input.shortDescription]) {
        ...
        if (!cleaned)
            continue;
        if (GENERIC_MODEL_VALUES.has(normalizeComparableText(cleaned)))
            continue;
...
export function getListingTextSources(listing) {
    const sources = [
    ...
    return sources.filter((source) => Boolean(source.text?.trim()));
}
...
export function buildListingReferenceText(listing) {
