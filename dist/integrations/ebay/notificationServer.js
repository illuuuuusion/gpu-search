...
function isNotificationServerEnabled() {
...
async function getNotificationPublicKey(keyId) {
    ...
    const response = await axios.get(`https://api.ebay.com/commerce/notification/v1/public_key/${encodeURIComponent(keyId)}`, {
        headers: {
...
async function verifyNotificationSignature(rawBody, signatureHeader) {
    ...
    try {
        ...
        verifier.end();
        const verified = verifier.verify(createPublicKey(publicKey), Buffer.from(decodedHeader.signature, 'base64'));
...
async function processDeletionNotification(rawBody, signatureHeader) {
...
export async function startEbayNotificationServer() {
