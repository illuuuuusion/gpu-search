...
function createProvider(importData, name = 'vlr') {
    return {
        name,
        importData,
    };
}
async function seedState(repository) {
    const state = {
        version: 2,
        metadata: {
            provider: 'vlr',
...
test('runSync returns degraded failed result for retryable network errors and preserves existing data', async () => {
    ...
    let attempts = 0;
    const service = new ValorantSyncService(repository, createProvider(async () => {
        attempts += 1;
        const error = new Error('getaddrinfo ENOTFOUND www.vlr.gg');
