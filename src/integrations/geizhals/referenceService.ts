...
function buildQuery(profile: GpuProfile): string {
...
async function collectFamilyLinks(page: Page, profile: GpuProfile): Promise<Array<{ title: string; url: string }>> {
  ...
  for (const link of links) {
    ...
    if (!aliases.some(alias => normalizedTitle.includes(alias))) continue;
...
async function parseFamilyPage(page: Page, familyLink: { title: string; url: string }): Promise<MarketReferenceFamily | null> {
