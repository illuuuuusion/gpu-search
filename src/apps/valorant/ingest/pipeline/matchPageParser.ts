...
import type { CachedMatchPage, CompositionRecord, MatchPageArtifact } from '../../domain/models.js';

const MAX_MAPS_PER_MATCH = 5;
const TEAM_SLOTS = [1, 2] as const;
const AGENT_SLOTS = [1, 2, 3, 4, 5] as const;

const AGENT_ALIAS_INDEX = new Map<string, string>();
...
const MAP_ALIAS_INDEX = new Map<string, string>();
...
function normalizeAgentKey(rawValue: string | undefined): string | undefined {
  if (!rawValue) {
    return undefined;
  }
...
function normalizeMapKey(rawValue: string | undefined): string | undefined {
  if (!rawValue) {
    return undefined;
  }
...
function isUnplayedMapValue(rawValue: string | undefined): boolean {
  if (!rawValue) {
    return true;
  }
...
function parseTemplateParameters(wikitext: string): Map<string, string> {
...
function parseWinner(rawValue: string | undefined, teamSlot: 1 | 2, scoreValue: string | undefined): boolean {
...
function parseTeamAgents(parameters: Map<string, string>, mapIndex: number, teamSlot: 1 | 2): string[] {
...
function readTeamName(parameters: Map<string, string>, cachedMatchPage: CachedMatchPage, teamSlot: 1 | 2): string | undefined {
  const directValue = stripWikiMarkup(
    parameters.get(teamSlot === 1 ? 'team1' : 'team2')
...
export function parseCompositionsFromMatchPage(
  cachedMatchPage: CachedMatchPage,
  rawArtifact: MatchPageArtifact,
): CompositionRecord[] {
