import type { CompositionRecord, FullCompositionAggregate } from '../domain/models.js';

const WIN_RATE_PRIOR = 0.5;
const WIN_RATE_PRIOR_GAMES = 12;

function sortAgentKeys(agentKeys: string[]): string[] {
  return [...new Set(agentKeys)].sort((left, right) => left.localeCompare(right));
}

function buildAggregateId(mapName: string, agentKeys: string[]): string {
  return `${mapName}:${sortAgentKeys(agentKeys).join('|')}`;
}

export function buildFullCompositionAggregates(compositions: CompositionRecord[]): FullCompositionAggregate[] {
  const aggregates = new Map<string, FullCompositionAggregate>();
