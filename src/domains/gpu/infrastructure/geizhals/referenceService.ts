...
import { buildListingReferenceText, compactComparableText } from '../../domain/listingSignals.js';
...
const PROFILE_QUERY_OVERRIDES: Record<string, string[]> = {
  'Titan RTX': [
...
const REFERENCE_OVERRIDES: Record<string, {
  title: string;
  query: string;
  url: string;
  priceEur: number;
...
interface ChromiumLaunchCandidate {
  label: string;
  executablePath?: string;
}

type PlaywrightBrowserEngine = 'chromium' | 'firefox' | 'webkit';
...
function buildQueryCandidates(profile: GpuProfile): string[] {
...
function buildOverrideReference(profileName: string): MarketReference | undefined {
...
function dedupeComparableTexts(values: string[]): string[] {
  return Array.from(new Map(values
    .filter(Boolean)
    .map(value => [compactComparableText(value), value] as const))
    .values());
}
...
function extractGpuModelToken(value: string): string | undefined {
...
function buildProfileGpuModelTokens(profile: GpuProfile): string[] {
  return dedupeComparableTexts([profile.name, ...profile.aliases]
    .map(alias => extractGpuModelToken(alias))
    .filter((value): value is string => Boolean(value)));
}

function matchesProfileAlias(profile: GpuProfile, title: string): boolean {
...
function matchesNegativeAlias(profile: GpuProfile, title: string): boolean {
  const normalizedTitle = normalizeText(title);
...
function looksLikeWholeSystem(title: string): boolean {
...
function referenceRejectionReason(profile: GpuProfile, title: string): string | undefined {
...
function dedupeVariants(variants: MarketReferenceVariant[]): MarketReferenceVariant[] {
  ...
  for (const variant of variants) {
    ...
    if (!title || variant.lowestPriceEur <= 0) continue;
...
function filterReferenceVariants(profile: GpuProfile, variants: MarketReferenceVariant[]): MarketReferenceVariant[] {
  return dedupeVariants(variants)
    .filter(variant => !referenceRejectionReason(profile, variant.title));
}

function filterReferenceFamily(
  profile: GpuProfile,
  family: MarketReferenceFamily,
  allowFallbackWithoutVariants: boolean,
): MarketReferenceFamily | null {
...
function extractPrimaryPrice(bodyText: string): number {
...
function flattenJsonLdNodes(input: unknown): JsonObject[] {
...
function readOfferStats(offers: unknown): { priceEur?: number; offerCount?: number } {
  ...
  const priceEur = [lowPrice, directPrice, nested.priceEur]
    .find((value): value is number => value !== undefined && Number.isFinite(value) && value > 0);
...
async function readPageInnerText(page: Page): Promise<string> {
  return (await page.locator('body').evaluate(node => (node as HTMLElement).innerText ?? '').catch(() => '')) ?? '';
}

async function readSearchResultLinks(page: Page): Promise<Array<{ href: string; text: string; seedPriceEur?: number }>> {
  return page.locator('a[href]').evaluateAll((anchors) => {
    ...
    return anchors.map(anchor => {
      ...
      let seedPriceEur: number | undefined;
      let current: HTMLElement | null = anchor.parentElement;
      for (let depth = 0; current && depth < 8; depth += 1, current = current.parentElement) {
        const blockText = (current.innerText ?? '').replace(/\s+/g, ' ').trim();
        if (!blockText) continue;
        if (blockText.length > 600) break;
        if (/keine angebote/i.test(blockText)) continue;
        if (text && !blockText.includes(text)) continue;

        seedPriceEur = parsePrice(blockText);
        if (seedPriceEur) break;
      }

      return { href, text, seedPriceEur };
    });
  }).catch(() => [] as Array<{ href: string; text: string; seedPriceEur?: number }>);
}

async function parseJsonLdVariants(page: Page): Promise<MarketReferenceVariant[]> {
  const scriptContents = await page.locator('script[type="application/ld+json"]').evaluateAll((scripts) =>
    scripts
      .map(script => script.textContent ?? '')
      .filter(Boolean),
  ).catch(() => [] as string[]);

  const variants: MarketReferenceVariant[] = [];
...
function isFresh(reference: MarketReference): boolean {
...
function parseVariantSection(bodyText: string, familyTitle: string, fallbackPriceEur: number): MarketReferenceVariant[] {
...
async function waitForUsablePage(page: Page): Promise<void> {
...
async function collectFamilyLinks(page: Page, profile: GpuProfile): Promise<{
  query: string;
  links: Array<{ title: string; url: string; seedPriceEur?: number }>;
}> {
  ...
  for (const query of queryCandidates) {
    ...
    const deduped = new Map<string, { title: string; url: string; seedPriceEur?: number }>();
    const currentUrl = page.url();
    const currentTitle = ((await page.locator('h1').first().textContent().catch(() => '')) ?? '').trim();

    if (
      /-[av]\d+\.html$/.test(new URL(currentUrl).pathname) &&
      currentTitle &&
    ...
    for (const link of links) {
      ...
      if (!/https:\/\/geizhals\.(?:de|at|eu)\//.test(link.href)) continue;

      const pathname = new URL(link.href).pathname;
      if (!/-(?:a|v)\d+\.html$/.test(pathname)) continue;
      ...
      if (referenceRejectionReason(profile, link.text)) continue;
...
async function parseFamilyPage(
  page: Page,
  profile: GpuProfile,
  familyLink: { title: string; url: string; seedPriceEur?: number },
): Promise<MarketReferenceFamily | null> {
...
async function fetchGeizhalsReference(page: Page, profile: GpuProfile): Promise<MarketReference> {
  const { query, links: familyLinks } = await collectFamilyLinks(page, profile);
  const families: MarketReferenceFamily[] = [];
  ...
  const dedupedFamilies = Array.from(new Map(families.map(family => [family.url, family] as const)).values())
...
async function buildChromiumLaunchEnv(): Promise<Record<string, string>> {
  const envWithLibraries = { ...process.env } as Record<string, string>;
...
async function findBundledChromiumExecutable(): Promise<string | undefined> {
  try {
...
async function buildChromiumLaunchCandidates(): Promise<ChromiumLaunchCandidate[]> {
  const candidates: ChromiumLaunchCandidate[] = [];
...
function requestedBrowserEngines(): PlaywrightBrowserEngine[] {
  return env.GEIZHALS_BROWSER_ENGINE === 'auto'
    ? ['chromium', 'firefox', 'webkit']
    : [env.GEIZHALS_BROWSER_ENGINE];
}

async function buildBrowserLaunchPlans(): Promise<BrowserLaunchPlan[]> {
...
async function openReferencePage(): Promise<{ browser: Browser; page: Page }> {
...
export class GeizhalsReferenceService implements MarketReferenceReader {
  private readonly references = new Map<string, MarketReference>();
  private refreshPromise: Promise<void> | null = null;
  private refreshTimer: NodeJS.Timeout | null = null;
  private cacheLoaded = false;
  private challengeCooldownUntil?: string;
