import type { EbayListing, RepairabilityAssessment } from '../domain/models.js';
...
const POSITIVE_SIGNALS: WeightedPattern[] = [
  {
...
const NEGATIVE_SIGNALS: WeightedPattern[] = [
  {
...
function buildAssessmentText(listing: EbayListing): string {
...
function applySignals(text: string, signals: WeightedPattern[]): { scoreDelta: number; reasons: string[] } {
  let scoreDelta = 0;
...
function resolveConfidence(reasons: string[]): RepairabilityAssessment['confidence'] {
  if (reasons.length >= 3) {
    return 'high';
  }
...
export function assessRepairability(listing: EbayListing): RepairabilityAssessment {
