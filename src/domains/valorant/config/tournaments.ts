import type { LiquipediaTournamentSeed } from '../domain/models.js';

export const VALORANT_TOURNAMENT_SEEDS: readonly LiquipediaTournamentSeed[] = [
  {
...
export function renderPageTitleTemplate(template: string, year: number): string {
  return template.replaceAll('{year}', String(year));
}

export function renderSearchQuery(template: string, year: number): string {
  return template.replaceAll('{year}', String(year));
}

export function getDiscoveryYears(windowDays: number, now = new Date()): number[] {
