...
import type { ValorantAppState } from '../domain/models.js';
...
interface MapAgentRate {
  total: number;
  countByAgent: Map<string, number>;
}

function buildMapAgentRates(state: ValorantAppState): Map<string, MapAgentRate> {
  const rates = new Map<string, MapAgentRate>();

  for (const composition of state.compositions) {
    const entry = rates.get(composition.mapName) ?? {
      total: 0,
      countByAgent: new Map<string, number>(),
    };
    entry.total += 1;
...
export function summarizeMetaChanges(previousState: ValorantAppState, nextState: ValorantAppState): string[] {
