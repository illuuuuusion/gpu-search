...
import type { EbayListing, EvaluatedListing, GpuProfile, ListingHealth, OfferType } from '../types/domain.js';
...
function resolveHealth(title: string): { health: ListingHealth; reasons: string[] } {
...
function acceptedForOfferType(offerType: OfferType, listing: EbayListing, profile: GpuProfile, health: ListingHealth): boolean {
...
function scoreListing(listing: EbayListing, profile: GpuProfile): number {
  const pricePerGb = listing.totalEur / profile.vramGb;
  return Number((1000 / Math.max(pricePerGb, 1)).toFixed(2));
}

export function evaluateListing(profile: GpuProfile, listing: EbayListing): EvaluatedListing {
