...
async function findExecutable(prefix: string, relativePath: string): Promise<string | undefined> {
  try {
...
async function inspectExecutable(label: string, executablePath: string | undefined): Promise<void> {
  if (!executablePath) {
...
async function main(): Promise<void> {
  ...
  await inspectExecutable(
    'chromium',
    await findExecutable('chromium-', 'chrome-linux64/chrome'),
  );
  await inspectExecutable(
    'chromium_headless_shell',
    await findExecutable('chromium_headless_shell-', 'chrome-headless-shell-linux64/chrome-headless-shell'),
  );
  await inspectExecutable(
    'firefox',
    await findExecutable('firefox-', 'firefox/firefox'),
  );
  await inspectExecutable(
    'webkit',
    await findExecutable('webkit-', 'pw_run.sh'),
  );
}
